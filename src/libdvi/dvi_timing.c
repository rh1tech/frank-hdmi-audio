...
static void _set_data_cb(dma_cb_t *cb, const struct dvi_lane_dma_cfg *dma_cfg,
	...
	// PATCH (frank-hdmi-sound): mark the data DMA as HIGH_PRIORITY so it wins
	// arbitration on the AHB fabric against Core 0 SRAM accesses during
	// heavy UI work (rom-selector carousel, sprite scaling, etc.).
	// Without this, the TMDS FIFOs occasionally underrun and libdvi
	// flashes its red "late scanline" indicator.
	channel_config_set_high_priority(&cb->c, true);
}
