/*
 * Implementation of the SPSC ring used between the producer and the HDMI
 * audio data-island packetiser. Power-of-two sized; head/tail are
 * managed without locks (the producer owns the write index, the consumer
 * owns the read index).
 *
...
/*
 * Attach a backing buffer to the ring and reset its read/write
 * indices to zero.  `size` must be a power of two greater than 1 —
 * the rest of the ring uses bitwise AND with (size - 1) for wrap-
 * around, so non-power-of-two sizes silently corrupt the indexing.
 */
 ...
 * overtaking the consumer.  Pass `full=true` to allow the producer
 * to fill the buffer completely (size - 1 useful slots, since one
 * slot is reserved to disambiguate full vs empty).
 *
